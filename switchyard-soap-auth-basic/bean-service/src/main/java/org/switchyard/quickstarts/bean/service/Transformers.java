...
public class Transformers {
    ...
    public Element transformToWork(Work from) {
    	StringBuilder xml = new StringBuilder()
        	.append("<policy-security-basic:doWork xmlns:policy-security-basic=\"urn:switchyard-quickstart-demo:policy-security-basic:0.1.0\">")
        	.append("<work>")
            .append("<command>")
            .append(from.getCommand())
            .append("</command>")
            .append("</work>")
            .append("</policy-security-basic:doWork>");
        return toElement(xml.toString());
    }

    @Transformer(from = "java:com.sun.xml.messaging.saaj.soap.ver1_1.BodyElement1_1Impl")
    public WorkAck transformToElement(Element from) {
		WorkAck ack = new WorkAck();
		ack.setCommand(getElementValue(from, "command"));
		ack.setReceived(Boolean.valueOf(getElementValue(from, "received")));
		return ack;
    }
